...
.badge {display: inline-block;padding: 0.25rem 0.65rem;border-radius: 999px;background: #e0f2fe;color: #075985;font-size: 0.8rem;margin-right: 0.35rem;}
    """,
...
def build_schema_from_reference(df_ref: Optional[pd.DataFrame]) -> Dict[str, Dict]:
    ...
    if df_ref is None:
        ...
        for c in EXPECTED_FEATURES:
            if c in CATEGORICAL_COLS:
                schema[c] = {
                    "type": "categorical",
                    "options": [0, 1] if c == "Gender" else [0, 1, 2, 3],
                }
    ...
    for c in EXPECTED_FEATURES:
        ...
        if c in CATEGORICAL_COLS:
            schema[c] = {
                "type": "categorical",
                "options": sorted([int(x) for x in col.dropna().unique().tolist()]),
            }
        elif c in BINARY_COLS:
            schema[c] = {"type": "binary", "options": [0, 1]}
...
def validate_and_prepare_csv(
    df_in: pd.DataFrame, schema: Dict[str, Dict]
    ...
    extra = [
        c
        for c in df.columns
        if c not in EXPECTED_FEATURES + IDENTIFIER_COLS + [OPTIONAL_TARGET]
    ]
    if missing:
    ...
    for c in EXPECTED_FEATURES:
        ...
        if spec["type"] in {"float", "int"}:
            ...
            if spec["type"] == "int":
                ...
                if len(bad) > 0:
                    issues.append(
                        f"{c}: hay valores no enteros en filas {bad.index.tolist()[:10]}"
                    )
                s = s.round(0)
            ...
            if len(bad_range) > 0:
                issues.append(
                    f"{c}: valores fuera de rango [{low}, {high}] en filas {bad_range.index.tolist()[:10]}"
                )
            out[c] = s
        else:
            ...
            if len(bad_cat) > 0:
                issues.append(
                    f"{c}: valores no permitidos {sorted(set(bad_cat.tolist()))[:10]}"
                )
            out[c] = s.round(0)
    ...
    for c in EXPECTED_FEATURES:
        ...
        if spec["type"] in {"float", "int"}:
            validity[f"{c}_valid"] &= (out[c].astype(float) >= spec["min"]) & (
                out[c].astype(float) <= spec["max"]
            )
        else:
            validity[f"{c}_valid"] &= out[c].isin(spec["options"])
    validity["row_valid"] = validity[
        [f"{c}_valid" for c in EXPECTED_FEATURES]
    ].all(axis=1)
...
def interpret_model(name, metrics, cm):
    ...
    if metrics["precision"] > metrics["recall"]:
        parts.append(
            "El modelo es más estricto al predecir positivos: comete menos falsos positivos, pero puede dejar pasar algunos casos reales."
        )
    elif metrics["recall"] > metrics["precision"]:
        parts.append(
            "El modelo detecta mejor los positivos reales: reduce falsos negativos, lo cual es útil cuando no conviene dejar pasar casos positivos."
        )
    else:
    ...
    if fn < fp:
        parts.append(
            "En este caso, los falsos negativos son menores que los falsos positivos, lo cual suele ser favorable en problemas médicos."
        )
    elif fp < fn:
        parts.append(
            "Hay más falsos negativos que falsos positivos, así que convendría revisar si el modelo está dejando pasar casos positivos."
        )
    else:
...
def predict_cluster(artifact, x: pd.DataFrame) -> Tuple[np.ndarray, pd.DataFrame]:
    ...
    if isinstance(artifact, dict):
        ...
        if pre is not None and cluster_model is not None:
            x_t = pre.transform(x)
            labels = (
                cluster_model.predict(x_t)
                if hasattr(cluster_model, "predict")
                else cluster_model.fit_predict(x_t)
            )
...
if reference_df is None:
    st.warning(
        "No se encontró el dataset de referencia. La validación usará rangos genéricos."
    )
else:
    st.success(
        f"Dataset de referencia cargado: {reference_df.shape[0]:,} filas × {reference_df.shape[1]:,} columnas"
    )
...
with tab1:
    ...
    uploaded = st.file_uploader(
        "Sube un archivo CSV con las variables del modelo", type=["csv"]
    )
    ...
    if uploaded is not None:
        try:
            ...
            st.success(
                f"Archivo cargado correctamente: {df_raw.shape[0]:,} filas × {df_raw.shape[1]:,} columnas"
            )
...
with tab2:
            ...
            for c in EXPECTED_FEATURES:
                if c in reference_df.columns:
                    if (
                        ...
                        or c in BINARY_COLS
                        or pd.api.types.is_integer_dtype(reference_df[c])
                    ):
                        defaults[c] = int(
                            pd.Series(reference_df[c]).mode(dropna=True).iloc[0]
                        )
                    else:
        ...
        def slider_or_number(col, key, spec):
            ...
            if spec["type"] in {"binary", "categorical"}:
                ...
                return col.selectbox(
                    label,
                    opts,
                    index=opts.index(defaults.get(key, opts[0]))
                    if defaults.get(key, opts[0]) in opts
                    else 0,
                )
            elif spec["type"] == "int":
                ...
                return col.number_input(
                    ...
                    min_value=mn,
                    max_value=mx,
                    value=float(defaults.get(key, mn)),
                    step=(mx - mn) / 100.0,
                )
    ...
    if submitted:
        ...
        st.session_state["manual_df"] = pd.concat(
            [st.session_state["manual_df"], new_row], ignore_index=True
        )
...
with tab3:
    ...
    run_btn = st.button(
        "Ejecutar predicción", type="primary", use_container_width=True
    )
    ...
    def get_input_data(source_choice: str) -> Optional[pd.DataFrame]:
        ...
        if (
            source_choice in ["CSV validado", "Ambos"]
            and csv_df is not None
            and len(csv_df) > 0
        ):
            parts.append(csv_df[EXPECTED_FEATURES].copy())

        if (
            source_choice in ["Registros manuales", "Ambos"]
            and man_df is not None
            and len(man_df) > 0
        ):
    ...
    if run_btn:
        ...
        if data_in is None or len(data_in) == 0:
            st.error(
                "No hay datos para predecir. Sube un CSV válido o agrega registros manuales."
            )
            ...
            st.success(
                f"Datos listos para predicción: {x_in.shape[0]:,} filas × {x_in.shape[1]:,} variables"
            )
            ...
            for model_name in selected_models:
                ...
                if model_name in [
                    ...
                    out[f"Diagnóstico_clínico_{model_name}"] = np.where(
                        clinical_pred == 1, "Tiene diagnóstico", "No tiene diagnóstico"
                    )
                    ...
                    if model_name == "Regresión logística":
                        ...
                        if coef is not None:
                            top = (
                                coef.sort_values("abs_coef", ascending=False)
                                .head(15)
                                .copy()
                            )
                            top["direction"] = np.where(
                                top["coef"] >= 0, "Positivo", "Negativo"
                            )
                            fig_coef = px.bar(
                                top.iloc[::-1],
                                x="coef",
                    ...
                    if source == "CSV validado":
                        ...
                        if (
                            ...
                            y_true = (
                                pd.to_numeric(
                                    uploaded_df[OPTIONAL_TARGET], errors="coerce"
                                )
                                .fillna(0)
                                .astype(int)
                                .values
                ...
                # ==================================================
                    ...
                    c1.metric(
                        ...
                        int(pd.Series(labels).nunique()),
                    )
                    c2.metric(
                        "Cluster más frecuente",
                        int(pd.Series(labels).mode().iloc[0]),
                    )
                    ...
                    fig_counts = px.bar(
                        counts, x="Cluster", y="Cantidad", title="Tamaño de clusters"
                    )
                        ...
                        compare_rows.append(
                            {
                                "Modelo": rf_name,
                                "Positivos predichos": (
                                    int((merged[diag_col] == "Tiene diagnóstico").sum())
                                    if diag_col in merged.columns
                                    else int((merged[pred_col] == 1).sum())
                                ),
                                "Negativos predichos": (
                                    int(
                                        (merged[diag_col] == "No tiene diagnóstico").sum()
                                    )
                                    if diag_col in merged.columns
                                    else int((merged[pred_col] == 0).sum())
                                ),
                                "Probabilidad media": (
                                    float(merged[prob_col].mean())
...
with tab4:
    ...
    if supervised_available:
        ...
        metric_cols = [
            ...
            if c in df_supervised.columns
        ]
        plot_df = df_supervised.melt(
            id_vars="Modelo",
        ...
        if df_supervised["Sensibilidad"].notna().any():
            ...
            for _, row in df_supervised.iterrows():
                ...
                if y_true is not None and len(y_true) == len(y_pred):
                    ...
                    st.plotly_chart(
                        plot_cm_heatmap(metrics["cm"], f"Matriz de confusión - {model_name}"),
                        use_container_width=True,
                    )
            ...
            st.warning(
                "No hay métricas reales disponibles para comparar porque no existe la columna Diagnosis en los datos evaluados."
            )
    ...
    for model_name, info in results.items():
        if info.get("kind") == "cluster":
            ...
            cluster_df = pd.DataFrame(
                {"Cluster": counts.index.astype(str), "Pacientes": counts.values}
            )
